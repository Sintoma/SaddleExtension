...
\end{equation}
Indeed, since $E$ is an open cone, there exists a circular cone inside $E$ with the same vertex, that we call $z$. Then, by moving this circular cone in the direction of its axis, that can be assumed to be $e_n = (0,...,0,1)$, we obtain a new open cone $E'\subset E$. Let us show \eqref{Eq:DistanceCones}. Since $E'\subset E$, it is enough to prove \eqref{Eq:DistanceCones} for $x\in \partial E$ and $ y\in\partial E'$. Hence, we have
$$x_n-z_n = C |x'-z'| \quad \text{ and } \quad y_n-z_n = t + C |y'-z'|, $$
for some positive constants $C$ and $t$. Here, we are using the notation $z = (z',z_n)$. Now, if we call $\sigma = |x'-z'|-|y'-z'|$, we get
\begin{align*}
|x-y|^2 &= |x'-y'|^2 + |x_n-y_n|^2 \geq \sigma^2 + (t^2 + C^2 \sigma^2 - 2tC\sigma)   \\
&= \left ( \sqrt{1+C^2} \sigma - \dfrac{Ct}{\sqrt{1+C^2}}\right )^2 + \dfrac{t^2}{1+C^2}\geq\dfrac{t^2}{1+C^2}.
\end{align*}
