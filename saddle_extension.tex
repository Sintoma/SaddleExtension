...
Let $u$ be the saddle-shaped solution of \eqref{Eq:AllenCahn} in dimension $2m\geq 14$. Consider the blow-down sequence $u_k(x) = u(kx)$ with $k\in \N$. On the one hand, since $u$ is stable in such dimensions and $\s\in(0,1/2)$, by Thereom~2.6 in \cite{CabreCintiSerra-Stable} there exists a subsequence $k_j$ such that
