...
\keywords{Fractional Allen-Cahn equation, saddle-shaped solution, Simons cone, nonlocal minimal surfaces, stability. \textit{2010 MSC}: 35J61, 35B08, 35B35}
...
\end{equation}
the conjecture was established by Savin \cite{Savin-DeGiorgi} (see also the previous work of Ghoussoub and Gui \cite{GhoussoubGuiOdd} in dimensions 4 and 5 for antisymmetric solutions). A counterexample to the conjecture in dimensions $n \geq 9$ was given by del~Pino, Kowalczyk and Wei \cite{delPinoKowalczykWei}. 
...
Let us remark that both sets $\Omega\in \R^{n+1}_+$ and $\Gamma\subset \partial_0\Omega$ play an important role in this notion of ``narrowness'', as illustrated in the following examples in $\R^2_+$. On the one hand, let $ \Omega_1 = \{\lambda>2(x-\varepsilon)\} \cap \{\lambda>-2(x+\varepsilon)\} \cap \{\lambda>0\}\subset \R^2_+ $ and let $\Gamma_1 = \partial_0 \Omega_1 = (-\varepsilon,\varepsilon)\subset \R$ ---see Figure 99(a). This pair has $R_0(\Omega_1,\Gamma_1,1/2) = +\infty$ for all $\varepsilon > 0$ even though $\Gamma_1$ is ``narrow'' in $\R$ in the usual sense if $\varepsilon$ is small enough. On the other hand, the pair consisting of $\Omega_2 = \{0<\lambda<\varepsilon\}$ and $\Gamma_2 = \partial_0 \Omega_2 = \R$ is ``extension-narrow'' if $\varepsilon$ is small enough, while $\Gamma_2$ is not ``narrow'' in the usual sense in $\R$ ---see Figure 99(b). 
...
\hspace{-0.35cm} \beqc{\PDEsystem}
u_{ss} + u_{tt} + u_{\lambda \lambda} + (m-1)\bpar{\dfrac{u_s}{s} + \dfrac{u_t}{t}} + \dfrac{a}{\lambda} u_\lambda  & = & 0& \text{ in } \{st>0,\ \lambda > 0\}, \\
...
$$
\partial_s U (x,0) = \dfrac{1}{\sqrt{2}} \partial_x u_0\bpar{\dfrac{s-t}{\sqrt{2}}, 0} \geq 0 \quad \text{ and } \lim_{R\to +\infty} \norm{(u_s- \partial_s U)(\cdot,0)}_{L^\infty(\R^{2m}\setminus B_{R})} = 0\,.
$$
...
Indeed, 
$$
\partial_t U (x,0) = -\dfrac{1}{\sqrt{2}} \partial_1 u_0\bpar{\dfrac{s-t}{\sqrt{2}}, 0} \leq 0 \quad \text{ and } \lim_{R\to +\infty} \norm{(u_t - \partial_t U) (\cdot,0) }_{L^\infty(\R^{2m}\setminus B_{R})} = 0\,.
$$
...
since
$$
U_{st} (x,0) = -\dfrac{1}{2} \partial_1^2 u_0\bpar{\dfrac{s-t}{\sqrt{2}}, 0} \geq 0 \quad \text{ and } \lim_{R\to +\infty} \norm{(u_{st} - U_{st})(\cdot,0) }_{L^\infty(\R^{2m}\setminus B_{R})} = 0\,.
$$
