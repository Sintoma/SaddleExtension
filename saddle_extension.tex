...
\end{equation}
Indeed, since $E$ is an open cone, there exists a circular cone inside $E$ with the same vertex, that we call $z$. Then, by moving this circular cone in the direction of its axis, that can be assume to be $e_n = (0,...,0,1)$, we obtain a new open cone $E'\subset E$ satisfying \eqref{Eq:DistanceCones}. That is, if $x\in E^c$ and $ y\in E'$, then
$$x_n-z_n < C |x'-z'| \quad \text{ and } \quad y_n-z_n \geq t + C |y'-z'|, $$
for some positive constants $C$ and $t$. Here, we are using the notation $z = (z',z_n)$. Now, we can distinguish two cases:
\begin{itemize}
	\item If $ \big||y'-z'|-|x'-z'| \big| \leq \frac{t}{2C} $, then
	\begin{align*}
		|y-x| &\geq y_n-x_n \geq t + C(|y'-z'|-|x'-z'|) \\ 
		&\geq t + C(-\big||y'-z'|-|x'-z'| \big| ) \geq t+C\left(-\frac{t}{2C} \right) = \frac{t}{2}.
	\end{align*}
	\item If $ \big||y'-z'|-|x'-z'| \big| \geq \frac{t}{2C} $, then
	\begin{align*}
	|y-x| &\geq |y'-x'| \geq \big||y'-z'|-|x'-z'| \big| = \frac{t}{2C}.
	\end{align*}
\end{itemize}
Therefore, $ |x-y|\geq t/2\,\min\{1,1/C\} >0$ and \eqref{Eq:DistanceCones} is satisfied.
