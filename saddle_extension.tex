...
\begin{definition}[``Downstairs-narrow'' domains]
	Let $H \subset \R^{n+1}_+$ be an open set, not necessarily bounded, and let $\Gamma_\mathrm{N} \subset \partial_0 H$ be a nonempty subset. Given $\theta \in (0,1)$ and $a\in (-1,1)$, we define $R_a(H,\Gamma,\theta) \in (0, +\infty]$ to be the smallest positive constant $R$ for which
	\begin{equation}
	\label{Eq:Narrow}
...
\begin{proof}[Proof of Proposition~\ref{Prop:MaxPrincipleNarrow}] 
	...
	$$
	\dfrac{\partial v}{\partial \nu^a} \leq - c v  \leq \norm{c_-}_{L^\infty (\partial_0H)} v \leq  \norm{c_-}_{L^\infty (\Gamma_+)} M \leq \dfrac{\partial \phi}{\partial \nu^a}
	$$
	if we choose $c_1 > d_\s\norm{c_-}_{L^\infty (\Gamma_+)}$\,.
	...
	\end{align*}
	Here we have used the definition of $R_a( H,\Gamma,\theta)$, the fact that $w \equiv M$ in $B^+_R(x_k)\setminus H_+$, the scaling properties of $|\cdot |_a$ and the weak Harnack inequality \eqref{Eq:WeakHarnack}. Now, if $c_1 \overline{c}_{n,\s} R^{2\s} < 1$, then $w(x_k) = M - v(x_k) + \phi(x_k)$ and therefore
	$$
	\theta^{1/p} M \leq  2^{\frac{n+1+a}{p}}|B_1^+|_a^{-1/p} C_h \{M - v(x_k) + \phi(x_k) \} \leq 2^{\frac{n+1+a}{p}}|B_1^+|_a^{-1/p} C_h \{1/k + c_1 M \overline{c}_{n,\s} R^{2\s} \}\,.
	$$
	...
	M \leq \dfrac{C}{k}
	$$
	for some positive constant $C$ independent of $k$. Since this holds for every $k>0$, we conclude that $M \leq 0$, which contradicts \eqref{Eq:PositiveSup}. Therefore, our initial assumption stating that $ H_+$ is not empty is false. This means that $v\leq 0$ in $H$.
\end{proof}
