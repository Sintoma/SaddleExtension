...
Note that if in \eqref{Eq:Narrow} we consider $a=0$ and full balls centered at every point $x\in \Omega$, we recover the usual definition of ``narrow'' domain. Here, instead, we only consider half-balls centered at points $x\in \Gamma \subset \partial_0\Omega$.

Once the quantity  $R_a(\Omega,\Gamma, \theta)$ is defined, we can state precisely the maximum principle in ``downstairs-narrow'' domains.
...
As a consequence of Proposition~\ref{Prop:MaxPrincipleNarrow}, next we establish that the maximum principle holds in sets $ \Omega \subset \ocal \times (0, +\infty) \subset \R^{2m+1}_+$ with $\Gamma \subset \partial_0  \Omega$ contained in an $\varepsilon$-neighborhood of the Simons cone.
...
Now, by the hypothesis at infinity on $v$ and the fact that $u>\delta$ in $\ocal_{\varepsilon}$, we have that $S$ is attained at some point  $(x_0,0)\in\partial_0\Omega \subset \ocal$. At this point we have
\begin{equation}
